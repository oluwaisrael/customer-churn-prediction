...
if st.button("Run Churn Analysis", type="primary"):
    ...
    if prediction == 1:
        st.error(f"**High Churn Risk!** Probability: {prediction_proba:.1%}")
        st.write("Action Required: Please prioritize retention offers.")
    else:
        st.success(f"**Low Churn Risk.** Probability: {prediction_proba:.1%}")
        st.write("Customer account is  stable.")